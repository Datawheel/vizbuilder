 ...
 */
export function chartTitleGenerator(chart: Chart, uiParams: UIParams) {
  ...
  title = translate("title.measure_and_modifier", {
    modifier: getAggregationTypeQualifier(measureSet.measure.aggregator, translate),
...
function arrayToSentence(strings: string[], translate: TranslateFunction): string {
 ...
 */
function getAggregationTypeQualifier(aggType: Aggregator, translate: TranslateFunction) {
  const qualifier =
    typeof aggType === "string" &&
    aggType &&
    translate(`aggregators.${aggType.toLowerCase()}`);
 ...
 */
function periodToString(start: string, end: string) {
