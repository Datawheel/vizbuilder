 ...
 */
export function tooltipGenerator(chart: Chart, {translate: t}: UIParams) {
  ...
  const bodyFn = input => {
    const output: [string, (item: Record<string, number>) => string][] = [];

    if (shouldShow.lci && shouldShow.uci) {
      output.push([
        t("chart_labels.ci"),
        d =>
          `${formatter(d[lciName] * 1 || 0, locale)} - ${formatter(d[uciName] * 1 || 0, locale)}`,
      ]);
...
type MetaMeasures = "moe" | "lci" | "uci" | "src" | "clt";
...
function areMetaMeasuresZero(
  ...
  while (n--) {
    ...
    results.src = results.src || !!item[srcName];
    results.clt = results.clt || !!item[cltName];
  }
  return results;
}
